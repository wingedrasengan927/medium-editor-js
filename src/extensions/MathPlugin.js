		...
		editor.registerCommand(
			...
			() => {
				...
				if ($isNodeSelection(selection)) {
					...
					selection
						.getNodes()
						.forEach((node) => nodesToExclude.add(node.getKey()));
				}
		...
		editor.registerCommand(
			...
			() => {
				...
				if ($isMathNode(adjacentNode) && adjacentNode.isInline()) {
					...
					const mathHighlightNode =
						$createMathHighlightNodeInline(cleanEquation);
					...
					((selection.anchor.getNode().getKey() ===
						...
						(lastChild &&
							...
							selection.anchor.getNode().getKey() ===
								lastChild.getKey() &&
							selection.anchor.offset ===
								lastChild.getTextContent().length));
		...
		editor.registerCommand(
			...
			() => {
				...
				if ($isMathNode(adjacentNode) && !adjacentNode.isInline()) {
					...
					const mathHighlightNode =
						$createMathHighlightNodeBlock(cleanEquation);
					adjacentNode.replace(mathHighlightNode);
					mathHighlightNode.select();
					return true;
				}
